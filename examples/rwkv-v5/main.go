...
import (
	...
	"mlgo/examples/rwkv-v5/rwkv_v5_go"
	// "mlgo/ml"
)
...
func main() {
	...
	var opts struct {
		...
		Model   string  `long:"model" description:"Path and file name of converted .bin RWKV-v5 model"`
	...
	_, err = rwkv_v5.LoadModel(params.modelPath, opts.Silent)
	// if err != nil {
	// 	_, err := rwkv_v5.Colorize("\n[magenta][ ERROR ][white] Failed to load model [light_magenta]\"%s\"\n\n", params.modelPath)
	// 	if err != nil {
	// 		return
	// 	}
	// 	os.Exit(0)
...
func showLogo() {
	...
	_, err = rwkv_v5.Colorize(logoColored)
	if err != nil {
		return
	}
	_, err = rwkv_v5.Colorize("\n\n   [magenta]▒▒▒▒[light_magenta] [ RWKV_v5.go v" + versionStr + " ] [light_blue][ RWKV V5 in pure Golang - based on rwkv.cpp ] [magenta]▒▒▒▒\n\n")
	if err != nil {
		return
	}
}
