...
import (
	...
	llama "mlgo/examples/mistral/llama_go"
	"mlgo/ml"
)
...
func main() {
	...
	var opts struct {
		...
		FType   string  `long:"ftype" description:"ftype of the model [gguf, ggjt]"`
	...
	for remainCount != 0 || params.interactive {
		...
		if len(embd) > 0 {
			...
			if pastCount+uint32(len(embd)) > params.ctxSize {
				fmt.Printf("[WARN] CONTEXT SWAPPING")
		...
		if !inputNoEcho {
			for _, id := range embd {
				...
				token := ml.Token2Str(ctx.Vocab, id)
				// fmt.Println("[INFO] Displaying token", token, len(final))
				final += token
